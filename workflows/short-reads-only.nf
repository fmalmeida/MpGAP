...
workflow sreads_only_nf {
  ...
  if (!params.skip_shovill && !params.shortreads_single && params.shortreads_paired) {
    preads.combine(prefix_ch).combine(Channel.from('spades', 'skesa', 'megahit')).view()
    shovill_sreads_assembly(preads.combine(prefix_ch).combine(Channel.from('spades', 'skesa', 'megahit')))
