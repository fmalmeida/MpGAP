...
process shovill {
  ...
  input:
  tuple val(id), val(entrypoint), file(sread1), file(sread2), file(single), file(lreads), val(lr_type), file(fast5), val(fast5_dir), file(bams), val(nBams), val(prefix), val(assembler)
