...
Please be aware, this sets the maximum amount of resources a single job can request to avoid surpassing your system limits. These resources are set dynamically by nextflow https://nf-co.re/usage/configuration#tuning-workflow-resources (we implemented the same principle).

Also, by its nature, nextflow tries to execute as much as he can in parallel, so even you turn these parameters to 4 cpus, for example, if your machine hase 12 threads, nextflow may try to run 3 jobs in parallel, each with 4 threads. To limit the number of parallel jobs that nextflow launches at a time, you must configure in a custom config file as shown here: https://www.nextflow.io/docs/latest/config.html#scope-executor.

.. note::
  
  Ps. the "local" executor is nextflow's default, which will the reality of most users. Limiting parallel jobs in local executor would look like this ``executor.$local.queueSize = 1``. Please, read NF manual about it.
